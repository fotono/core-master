...
Slot::Slot(uint64 slotIndex, FCP& fcp)
    : mSlotIndex(slotIndex)
    , mSCP(fcp)
    , mBallotProtocol(*this)
    , mNominationProtocol(*this)
    , mFullyValidated(fcp.getLocalNode()->isValidator())
{
}
...
Slot::processEnvelope(SCPEnvelope const& envelope, bool self)
{
    ...
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "Slot::processEnvelope"
                           << " i: " << getSlotIndex() << " "
                           << mSCP.envToStr(envelope);

    FCP::EnvelopeState res;
    ...
    catch (...)
    {
        ...
        CLOG(ERROR, "FCP") << "Exception in processEnvelope "
