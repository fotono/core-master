...
TEST_CASE("vblocking and quorum", "[fcp]")
{
...
TEST_CASE("v-blocking distance", "[fcp]")
{
    SIMULATION_CREATE_NODE(0);
    SIMULATION_CREATE_NODE(1);
    SIMULATION_CREATE_NODE(2);
...
TEST_CASE("ballot protocol core5", "[fcp][ballotprotocol]")
{
    SIMULATION_CREATE_NODE(0);
    SIMULATION_CREATE_NODE(1);
    SIMULATION_CREATE_NODE(2);
    ...
    TestSCP fcp(v0SecretKey.getPublicKey(), qSet);

    fcp.storeQuorumSet(std::make_shared<SCPQuorumSet>(qSet));
    uint256 qSetHash0 = fcp.mSCP.getLocalNode()->getQuorumSetHash();

    REQUIRE(xValue < yValue);

    CLOG(INFO, "FCP") << "";
    CLOG(INFO, "FCP") << "BEGIN TEST";
    ...
    auto recvVBlockingChecks = [&](genEnvelope gen, bool withChecks) {
        ...
        size_t i = fcp.mEnvs.size();
        fcp.receiveEnvelope(e1);
        if (withChecks)
        {
            REQUIRE(fcp.mEnvs.size() == i);
        }
        i++;
        fcp.receiveEnvelope(e2);
        if (withChecks)
        {
            REQUIRE(fcp.mEnvs.size() == i);
        }
    ...
    auto recvQuorumChecksEx = [&](genEnvelope gen, bool withChecks,
        ...
        fcp.receiveEnvelope(e1);
        fcp.receiveEnvelope(e2);
        size_t i = fcp.mEnvs.size() + 1;
        fcp.receiveEnvelope(e3);
    ...
    auto nodesAllPledgeToCommit = [&]() {
        ...
        REQUIRE(fcp.bumpState(0, xValue));
        REQUIRE(fcp.mEnvs.size() == 1);

        verifyPrepare(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, b);

        fcp.receiveEnvelope(prepare1);
        REQUIRE(fcp.mEnvs.size() == 1);
        REQUIRE(fcp.mHeardFromQuorums[0].size() == 0);

        fcp.receiveEnvelope(prepare2);
        REQUIRE(fcp.mEnvs.size() == 1);
        REQUIRE(fcp.mHeardFromQuorums[0].size() == 0);

        fcp.receiveEnvelope(prepare3);
        REQUIRE(fcp.mEnvs.size() == 2);
        REQUIRE(fcp.mHeardFromQuorums[0].size() == 1);
        REQUIRE(fcp.mHeardFromQuorums[0][0] == b);

        // We have a quorum including us

        verifyPrepare(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, b, &b);

        fcp.receiveEnvelope(prepare4);
        REQUIRE(fcp.mEnvs.size() == 2);
        ...
        fcp.receiveEnvelope(prepared4);
        fcp.receiveEnvelope(prepared3);
        REQUIRE(fcp.mEnvs.size() == 2);

        fcp.receiveEnvelope(prepared2);
        REQUIRE(fcp.mEnvs.size() == 3);

        // confirms prepared
        verifyPrepare(fcp.mEnvs[2], v0SecretKey, qSetHash0, 0, b, &b, b.counter,
                      b.counter);

        // extra statement doesn't do anything
        fcp.receiveEnvelope(prepared1);
        REQUIRE(fcp.mEnvs.size() == 3);
    };
    ...
    SECTION("bumpState x")
    {
        ...
        verifyPrepare(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, expectedBallot);
    }

    SECTION("start <1,x>")
    {
        // no timer is set
        REQUIRE(!fcp.hasBallotTimer());
        ...
        REQUIRE(fcp.bumpState(0, aValue));
        REQUIRE(fcp.mEnvs.size() == 1);
        REQUIRE(!fcp.hasBallotTimer());
        ...
        SECTION("prepared A1")
        {
            ...
            REQUIRE(fcp.mEnvs.size() == 2);
            verifyPrepare(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, A1, &A1);
            ...
            SECTION("bump prepared A2")
            {
                ...
                REQUIRE(fcp.bumpState(0, aValue));
                REQUIRE(fcp.mEnvs.size() == 3);
                verifyPrepare(fcp.mEnvs[2], v0SecretKey, qSetHash0, 0, A2, &A1);
                REQUIRE(!fcp.hasBallotTimer());

                recvQuorumEx(makePrepareGen(qSetHash, A2), true);
                REQUIRE(fcp.mEnvs.size() == 4);
                verifyPrepare(fcp.mEnvs[3], v0SecretKey, qSetHash0, 0, A2, &A2);

                SECTION("Confirm prepared A2")
                {
                    recvQuorum(makePrepareGen(qSetHash, A2, &A2));
                    REQUIRE(fcp.mEnvs.size() == 5);
                    verifyPrepare(fcp.mEnvs[4], v0SecretKey, qSetHash0, 0, A2,
                                  &A2, 2, 2);
                    REQUIRE(!fcp.hasBallotTimerUpcoming());
                    ...
                    SECTION("Accept commit")
                    {
                        SECTION("Quorum A2")
                        {
                            ...
                            REQUIRE(fcp.mEnvs.size() == 6);
                            verifyConfirm(fcp.mEnvs[5], v0SecretKey, qSetHash0,
                                          0, 2, A2, 2, 2);
                            REQUIRE(!fcp.hasBallotTimerUpcoming());
                            ...
                            SECTION("Quorum prepared A3")
                            {
                                ...
                                REQUIRE(fcp.mEnvs.size() == 7);
                                verifyConfirm(fcp.mEnvs[6], v0SecretKey,
                                ...
                                recvQuorumEx(
                                    makePrepareGen(qSetHash, A3, &A2, 2, 2),
                                    true);
                                REQUIRE(fcp.mEnvs.size() == 8);
                                verifyConfirm(fcp.mEnvs[7], v0SecretKey,
                                ...
                                SECTION("Accept more commit A3")
                                {
                                    recvQuorum(makePrepareGen(qSetHash, A3, &A3,
                                                              2, 3));
                                    REQUIRE(fcp.mEnvs.size() == 9);
                                    verifyConfirm(fcp.mEnvs[8], v0SecretKey,
                                                  qSetHash0, 0, 3, A3, 2, 3);
                                    REQUIRE(!fcp.hasBallotTimerUpcoming());

                                    REQUIRE(fcp.mExternalizedValues.size() ==
                                            0);
                                    ...
                                    SECTION("Quorum externalize A3")
                                    {
                                        recvQuorum(makeConfirmGen(qSetHash, 3,
                                                                  A3, 2, 3));
                                        REQUIRE(fcp.mEnvs.size() == 10);
                                        verifyExternalize(fcp.mEnvs[9],
                                                          v0SecretKey,
                                                          qSetHash0, 0, A2, 3);
                                        REQUIRE(!fcp.hasBallotTimer());

                                        REQUIRE(
                                            fcp.mExternalizedValues.size() ==
                                            1);
                                        REQUIRE(fcp.mExternalizedValues[0] ==
                                                aValue);
                                    }
                                }
                        ...
                        SECTION("v-blocking")
                        {
                            SECTION("CONFIRM")
                            {
                                SECTION("CONFIRM A2")
                                {
                                    ...
                                    REQUIRE(fcp.mEnvs.size() == 6);
                                    verifyConfirm(fcp.mEnvs[5], v0SecretKey,
                    ...
                    SECTION("get conflicting prepared B")
                    {
                        SECTION("same counter")
                        {
                            ...
                            REQUIRE(fcp.mEnvs.size() == 6);
                            verifyPrepare(fcp.mEnvs[5], v0SecretKey, qSetHash0,
                                          0, A2, &B2, 0, 2, &A2);
                            REQUIRE(!fcp.hasBallotTimerUpcoming());
                        }
                        SECTION("higher counter")
                        {
                            ...
                            recvQuorumChecksEx(
                                ...
                                true, true);
                            REQUIRE(fcp.mEnvs.size() == 7);
                            verifyConfirm(fcp.mEnvs[6], v0SecretKey, qSetHash0,
                                          0, 3, B3, 2, 2);
                        }
                    }
                }
                SECTION("Confirm prepared mixed")
                {
                    ...
                    SECTION("mixed A2")
                    {
                        ...
                        REQUIRE(fcp.mEnvs.size() == 6);
                        verifyPrepare(fcp.mEnvs[5], v0SecretKey, qSetHash0, 0,
                                      A2, &B2, 0, 2, &A2);
                        REQUIRE(!fcp.hasBallotTimerUpcoming());
            ...
            SECTION("switch prepared B1")
            {
                ...
                REQUIRE(fcp.mEnvs.size() == 3);
                verifyPrepare(fcp.mEnvs[2], v0SecretKey, qSetHash0, 0, A1, &B1,
                              0, 0, &A1);
                REQUIRE(!fcp.hasBallotTimerUpcoming());
            }
        }
        SECTION("prepared B (v-blocking)")
        {
            recvVBlocking(makePrepareGen(qSetHash, B1, &B1));
            REQUIRE(fcp.mEnvs.size() == 2);
            verifyPrepare(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, A1, &B1);
            REQUIRE(!fcp.hasBallotTimer());
        }
        SECTION("confirm (v-blocking)")
        {
            SECTION("via CONFIRM")
            {
                ...
                REQUIRE(fcp.mEnvs.size() == 2);
                verifyConfirm(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, 3, A3, 3,
                              3);
                REQUIRE(!fcp.hasBallotTimer());
            }
            SECTION("via EXTERNALIZE")
            {
                ...
                REQUIRE(fcp.mEnvs.size() == 2);
                verifyConfirm(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0,
            ...
            SECTION("bump prepared A2")
            {
                SECTION("Confirm prepared A2")
                {
                    ...
                    SECTION("Quorum A2")
                    {
                        ...
                        recvQuorum(makePrepareGen(qSetHash, A2, &A2));
                        REQUIRE(fcp.mEnvs.size() == 1);
                        verifyPrepare(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0,
                                      A2, &A2, 1, 2);
                    }
                    SECTION("Quorum B2")
                    {
                    ...
                    SECTION("Accept commit")
                    {
                        SECTION("Quorum A2")
                        {
                            recvQuorum(makePrepareGen(qSetHash, A2, &A2, 2, 2));
                            REQUIRE(fcp.mEnvs.size() == 1);
                            verifyConfirm(fcp.mEnvs[0], v0SecretKey, qSetHash0,
                                          0, 2, A2, 2, 2);
                        }
                        SECTION("Quorum B2")
                        {
                            ...
                            SECTION("CONFIRM")
                            {
                                SECTION("CONFIRM A2")
                                {
                                    recvVBlocking(
                                        makeConfirmGen(qSetHash, 2, A2, 2, 2));
                                    REQUIRE(fcp.mEnvs.size() == 1);
                                    verifyConfirm(fcp.mEnvs[0], v0SecretKey,
                                                  qSetHash0, 0, 2, A2, 2, 2);
                                }
                                SECTION("CONFIRM A3..4")
                                {
        ...
        SECTION("confirm (v-blocking)")
        {
            SECTION("via CONFIRM")
            {
                ...
                fcp.receiveEnvelope(
                    makeConfirm(v2SecretKey, qSetHash, 0, 4, A4, 2, 4));
                REQUIRE(fcp.mEnvs.size() == 1);
                verifyConfirm(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, 3, A3, 3,
                              3);
            }
            SECTION("via EXTERNALIZE")
            {
                ...
                fcp.receiveEnvelope(
                    makeExternalize(v2SecretKey, qSetHash, 0, A3, 5));
                REQUIRE(fcp.mEnvs.size() == 1);
                verifyConfirm(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0,
                              UINT32_MAX, AInf, 3, UINT32_MAX);
            }
        }
    }
    ...
    SECTION("normal round (1,x)")
    {
        ...
        fcp.receiveEnvelope(preparedC1);
        fcp.receiveEnvelope(preparedC2);
        REQUIRE(fcp.mEnvs.size() == 3);

        // this should cause the node to accept 'commit b' (quorum)
        // and therefore send a "CONFIRM" message
        fcp.receiveEnvelope(preparedC3);
        REQUIRE(fcp.mEnvs.size() == 4);

        verifyConfirm(fcp.mEnvs[3], v0SecretKey, qSetHash0, 0, 1, b, b.counter,
                      b.counter);
        ...
        // those should not trigger anything just yet
        fcp.receiveEnvelope(confirm1);
        fcp.receiveEnvelope(confirm2);
        REQUIRE(fcp.mEnvs.size() == 4);

        fcp.receiveEnvelope(confirm3);
        // this causes our node to
        // externalize (confirm commit c)
        REQUIRE(fcp.mEnvs.size() == 5);

        // The slot should have externalized the value
        REQUIRE(fcp.mExternalizedValues.size() == 1);
        REQUIRE(fcp.mExternalizedValues[0] == xValue);

        verifyExternalize(fcp.mEnvs[4], v0SecretKey, qSetHash0, 0, b,
                          b.counter);

        // extra vote should not do anything
        fcp.receiveEnvelope(confirm4);
        REQUIRE(fcp.mEnvs.size() == 5);
        REQUIRE(fcp.mExternalizedValues.size() == 1);
        ...
        SECTION("bumpToBallot prevented once committed")
        {
            ...
            fcp.receiveEnvelope(confirm1b2);
            fcp.receiveEnvelope(confirm2b2);
            fcp.receiveEnvelope(confirm3b2);
            fcp.receiveEnvelope(confirm4b2);
            REQUIRE(fcp.mEnvs.size() == 5);
            REQUIRE(fcp.mExternalizedValues.size() == 1);
        }
    }
    ...
    SECTION("range check")
    {
        ...
        fcp.receiveEnvelope(confirm2);
        REQUIRE(fcp.mEnvs.size() == 5);
        verifyConfirm(fcp.mEnvs[4], v0SecretKey, qSetHash0, 0, 4,
                      SCPBallot(4, xValue), 2, 4);
        ...
        verifyExternalize(fcp.mEnvs[5], v0SecretKey, qSetHash0, 0,
    ...
    SECTION("timeout when h is set -> stay locked on h")
    {
        ...
        REQUIRE(fcp.mEnvs.size() == 3);
        verifyPrepare(fcp.mEnvs[2], v0SecretKey, qSetHash0, 0, bx, &bx,
                      bx.counter, bx.counter);

        // now, see if we can timeout and move to a different value
        REQUIRE(fcp.bumpState(0, yValue));
        REQUIRE(fcp.mEnvs.size() == 4);
        SCPBallot newbx(2, xValue);
        verifyPrepare(fcp.mEnvs[3], v0SecretKey, qSetHash0, 0, newbx, &bx,
                      bx.counter, bx.counter);
    }
    ...
    SECTION("timeout from multiple nodes")
    {
        ...
        REQUIRE(fcp.mEnvs.size() == 1);
        verifyPrepare(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, x1);

        recvQuorum(makePrepareGen(qSetHash, x1));
        // quorum -> prepared (1,x)
        REQUIRE(fcp.mEnvs.size() == 2);
        verifyPrepare(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, x1, &x1);
        ...
        // prepares (2,x)
        REQUIRE(fcp.mEnvs.size() == 3);
        verifyPrepare(fcp.mEnvs[2], v0SecretKey, qSetHash0, 0, x2, &x1);

        recvQuorum(makePrepareGen(qSetHash, x1, &x1));
        // quorum -> set nH=1
        REQUIRE(fcp.mEnvs.size() == 4);
        verifyPrepare(fcp.mEnvs[3], v0SecretKey, qSetHash0, 0, x2, &x1, 0, 1);
        REQUIRE(fcp.mEnvs.size() == 4);

        recvVBlocking(makePrepareGen(qSetHash, x2, &x2, 1, 1));
        // v-blocking prepared (2,x) -> prepared (2,x)
        REQUIRE(fcp.mEnvs.size() == 5);
        verifyPrepare(fcp.mEnvs[4], v0SecretKey, qSetHash0, 0, x2, &x2, 0, 1);
        ...
        //  -> we confirm c=h=x1
        REQUIRE(fcp.mEnvs.size() == 7);
        verifyPrepare(fcp.mEnvs[5], v0SecretKey, qSetHash0, 0, x2, &x2, 2, 2);
        verifyConfirm(fcp.mEnvs[6], v0SecretKey, qSetHash0, 0, 2, x2, 1, 1);
    }
    ...
    SECTION("timeout after prepare, receive old messages to prepare")
    {
        ...
        fcp.receiveEnvelope(makePrepare(v1SecretKey, qSetHash, 0, x1));
        fcp.receiveEnvelope(makePrepare(v2SecretKey, qSetHash, 0, x1));
        fcp.receiveEnvelope(makePrepare(v3SecretKey, qSetHash, 0, x1));
        ...
        // prepares (3,x)
        REQUIRE(fcp.mEnvs.size() == 4);
        verifyPrepare(fcp.mEnvs[3], v0SecretKey, qSetHash0, 0, x3, &x1);
        ...
        REQUIRE(fcp.mEnvs.size() == 5);
        verifyPrepare(fcp.mEnvs[4], v0SecretKey, qSetHash0, 0, x3, &x2);

        fcp.receiveEnvelope(
            makePrepare(v3SecretKey, qSetHash, 0, x2, &x2, 1, 2));
        // quorum -> set nH=2
        REQUIRE(fcp.mEnvs.size() == 6);
        verifyPrepare(fcp.mEnvs[5], v0SecretKey, qSetHash0, 0, x3, &x2, 0, 2);
    }
...
TEST_CASE("nomination tests core5", "[fcp][nominationprotocol]")
{
    SIMULATION_CREATE_NODE(0);
    SIMULATION_CREATE_NODE(1);
    SIMULATION_CREATE_NODE(2);
    ...
    SECTION("nomination - v0 is top")
    {
        ...
        {
            REQUIRE(fcp.nominate(0, xValue, false));

            std::vector<Value> votes, accepted;
            votes.emplace_back(xValue);

            REQUIRE(fcp.mEnvs.size() == 1);
            verifyNominate(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, votes,
                           accepted);
            ...
            // nothing happens yet
            fcp.receiveEnvelope(nom1);
            fcp.receiveEnvelope(nom2);
            REQUIRE(fcp.mEnvs.size() == 1);

            // this causes 'x' to be accepted (quorum)
            fcp.receiveEnvelope(nom3);
            REQUIRE(fcp.mEnvs.size() == 2);

            fcp.mExpectedCandidates.emplace(xValue);
            fcp.mCompositeValue = xValue;

            accepted.emplace_back(xValue);
            verifyNominate(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, votes,
                           accepted);

            // extra message doesn't do anything
            fcp.receiveEnvelope(nom4);
            REQUIRE(fcp.mEnvs.size() == 2);
            ...
            // nothing happens yet
            fcp.receiveEnvelope(acc1);
            fcp.receiveEnvelope(acc2);
            REQUIRE(fcp.mEnvs.size() == 2);

            fcp.mCompositeValue = xValue;
            // this causes the node to send a prepare message (quorum)
            fcp.receiveEnvelope(acc3);
            REQUIRE(fcp.mEnvs.size() == 3);

            verifyPrepare(fcp.mEnvs[2], v0SecretKey, qSetHash0, 0,
                          SCPBallot(1, xValue));

            fcp.receiveEnvelope(acc4);
            REQUIRE(fcp.mEnvs.size() == 3);
                    ...
                    "-> update latest to (z=x+y)")
            {
                ...
                fcp.receiveEnvelope(acc1_2);
                REQUIRE(fcp.mEnvs.size() == 3);

                // v-blocking
                fcp.receiveEnvelope(acc2_2);
                REQUIRE(fcp.mEnvs.size() == 4);
                verifyNominate(fcp.mEnvs[3], v0SecretKey, qSetHash0, 0, votes2,
                               votes2);

                fcp.mExpectedCandidates.insert(yValue);
                fcp.mCompositeValue = zValue;
                // this updates the composite value to use next time
                // but does not prepare it
                fcp.receiveEnvelope(acc3_2);
                REQUIRE(fcp.mEnvs.size() == 4);

                REQUIRE(fcp.getLatestCompositeCandidate(0) == zValue);

                fcp.receiveEnvelope(acc4_2);
                REQUIRE(fcp.mEnvs.size() == 4);
            }
        ...
        SECTION("self nominates 'x', others nominate y -> prepare y")
        {
            ...
            REQUIRE(fcp.mEnvs.size() == 1);
            verifyNominate(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, myVotes,
                           accepted);
            ...
            SECTION("others only vote for y")
            {
                ...
                verifyNominate(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, myVotes,
                               acceptedY);
            }
            SECTION("others accepted y")
            {
                ...
                fcp.mExpectedCandidates.clear();
                fcp.mExpectedCandidates.insert(yValue);
                fcp.mCompositeValue = yValue;
                // this causes the node to send a prepare message (quorum)
                fcp.receiveEnvelope(acc3);
                REQUIRE(fcp.mEnvs.size() == 3);
    ...
    SECTION("v1 is top node")
    {
        ...
        fcp.mPriorityLookup = [&](NodeID const& n) {
        ...
        fcp.mHashValueCalculator = [&](Value const& v) {
        ...
        SECTION("nomination waits for v1")
        {
            ...
            fcp.receiveEnvelope(nom1);
            REQUIRE(fcp.mEnvs.size() == 1);
            verifyNominate(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, votesY,
                           emptyV);
            ...
            SECTION("timeout -> pick another value from v1")
            {
                ...
                REQUIRE(fcp.nominate(0, zValue, true));
                // picks up 'x' from v1 (as we already have 'y')
                // which also happens to causes 'x' to be accepted
                REQUIRE(fcp.mEnvs.size() == 2);
                verifyNominate(fcp.mEnvs[1], v0SecretKey, qSetHash0, 0, votesXY,
                               votesX);
            }
        }
        SECTION("v1 dead, timeout")
        {
            ...
            SECTION("v0 is new top node")
            {
                ...
                REQUIRE(fcp.nominate(0, xValue, true));
                REQUIRE(fcp.mEnvs.size() == 1);
                verifyNominate(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, votesX,
                               emptyV);
            }
            SECTION("v2 is new top node")
            {
                ...
                REQUIRE(fcp.nominate(0, xValue, true));
                REQUIRE(fcp.mEnvs.size() == 1);
                verifyNominate(fcp.mEnvs[0], v0SecretKey, qSetHash0, 0, votesZ,
                               emptyV);
            }
