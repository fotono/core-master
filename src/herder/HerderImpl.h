...
class HerderImpl : public Herder
{
  public:
    ...
    void valueExternalized(uint64 slotIndex, fotonoValue const& value);
  ...
  private:
    ...
    // timer that detects that we're stuck on an FCP slot
    VirtualTimer mTrackingTimer;

    // saves the FCP messages that the instance sent out last
    void persistSCPState(uint64 slot);
    // restores FCP state based on the last messages saved on disk
