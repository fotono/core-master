...
FCP::FCP(SCPDriver& driver, NodeID const& nodeID, bool isValidator,
...
FCP::EnvelopeState
FCP::receiveEnvelope(SCPEnvelope const& envelope)
{
    // If the envelope is not correctly signed, we ignore it.
    if (!mDriver.verifyEnvelope(envelope))
    {
        CLOG(DEBUG, "FCP") << "FCP::receiveEnvelope invalid";
        return FCP::EnvelopeState::INVALID;
    }
...
bool
FCP::nominate(uint64 slotIndex, Value const& value, Value const& previousValue)
{
...
void
FCP::stopNomination(uint64 slotIndex)
{
...
void
FCP::updateLocalQuorumSet(SCPQuorumSet const& qSet)
{
    mLocalNode->updateQuorumSet(qSet);
}

SCPQuorumSet const&
FCP::getLocalQuorumSet()
{
...
void
FCP::purgeSlots(uint64 maxSlotIndex)
{
...
std::shared_ptr<Slot>
FCP::getSlot(uint64 slotIndex, bool create)
{
...
Json::Value
FCP::getJsonQuorumInfo(NodeID const& id, bool summary, uint64 index)
{
    Json::Value ret;
    if (index == 0)
    {
...
size_t
FCP::getKnownSlotsCount() const
{
    return mKnownSlots.size();
}

size_t
FCP::getCumulativeStatemtCount() const
{
    size_t c = 0;
...
FCP::getLatestMessagesSend(uint64 slotIndex)
{
...
void
FCP::setStateFromEnvelope(uint64 slotIndex, SCPEnvelope const& e)
{
...
std::vector<SCPEnvelope>
FCP::getCurrentState(uint64 slotIndex)
{
    auto slot = getSlot(slotIndex, false);
    if (slot)
    {
...
std::vector<SCPEnvelope>
FCP::getExternalizingState(uint64 slotIndex)
{
    auto slot = getSlot(slotIndex, false);
    if (slot)
    {
...
FCP::TriBool
FCP::isNodeInQuorum(NodeID const& node)
{
...
std::string
FCP::getValueString(Value const& v) const
{
    return mDriver.getValueString(v);
}

std::string
FCP::ballotToStr(SCPBallot const& ballot) const
{
...
std::string
FCP::ballotToStr(std::unique_ptr<SCPBallot> const& ballot) const
{
    std::string res;
    if (ballot)
    {
...
std::string
FCP::envToStr(SCPEnvelope const& envelope) const
{
    return envToStr(envelope.statement);
}

std::string
FCP::envToStr(SCPStatement const& st) const
{
    std::ostringstream oss;
