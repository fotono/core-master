...
#include "main/fotonoCoreVersion.h"
...
static const struct option fotono_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: fotono-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time fotono-core is run, FCP will "
          "start "
        ...
        Json::Value& catchupInfo)
{
    ...
    catch (std::invalid_argument const&)
    {
        ...
        LOG(INFO) << "* If you really want to catchup to " << to
                  << " run fotono-core with --newdb parameter.";
        LOG(INFO) << "*";
        return 2;
    }
...
setForceSCPFlag(Config const& cfg, bool isOn)
{
    ...
    if (checkInitialized(app))
    {
        ...
        {
            LOG(INFO) << "* ";
            LOG(INFO) << "* The `force fcp` flag has been set in the db.";
            LOG(INFO) << "* ";
            LOG(INFO)
                << "* The next launch will start fcp from the account balances";
        ...
        {
            LOG(INFO) << "* ";
            LOG(INFO) << "* The `force fcp` flag has been cleared.";
...
{
    LOG(INFO) << "Starting fotono-core " << fotono_CORE_VERSION;
...
main(int argc, char* const* argv)
{
    ...
    std::string cfgFile("fotono-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", fotono_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
        ...
        case OPT_VERSION:
            std::cout << fotono_CORE_VERSION << std::endl;
            return 0;
        case OPT_HELP:
        default:
            usage(0);
    ...
    try
    {
        ...
        if (cfg.MANUAL_CLOSE)
        {
            ...
            // (this is to avoid to force fcp all the time when testing)
            cfg.FORCE_SCP = true;
        }
    }
    catch (std::exception& e)
