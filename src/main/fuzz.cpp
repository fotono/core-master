 ...
 *     random fotonoMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and appplies it to a pair of of
 *     fotono-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(fotonoMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, fotonoMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing fotono-core " << fotono_CORE_VERSION;
...
genfuzz(std::string const& filename)
{
    ...
    out.open(filename);
    autocheck::generator<fotonoMessage> gen;
    for (size_t i = 0; i < n; ++i)
    {
        try
        {
            fotonoMessage m(gen(10));
            out.writeOne(m);
