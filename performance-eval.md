...
* Disk I/O at the operating system level (ie: unit is typically in blocks/s), both reads and writes of the fotono-core process
...
* FCP and flooding related changes typically require all validators to run code compatible with a change.
...
* The public network has a much more diverse set of validators than testnet, which allows better evaluation of FCP related metrics.
...
* Evaluating FCP and overlay for various network and trust topologies.
...
This scenario looks at the overhead of flooding transactions and FCP messages (required for transactions to be included in a ledger).

### Built-in load generator
fotono-core has a built-in load generator that allows to inject transactions on private networks.
