...
The `get` and `put` commands define subprocesses that `fotono-core` will
...
called `s3fotono` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3fotono]
get="curl http://history.fotono.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.fotono.org/{1}"
~~~~

In this example, `fotono-core` will use the `curl` command to fetch history and
