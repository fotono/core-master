...
HerderSCPDriver::SCPMetrics::SCPMetrics(Application& app)
    : mEnvelopeSign(
          app.getMetrics().NewMeter({"fcp", "envelope", "sign"}, "envelope"))
    , mEnvelopeValidSig(app.getMetrics().NewMeter(
          {"fcp", "envelope", "validsig"}, "envelope"))
    , mEnvelopeInvalidSig(app.getMetrics().NewMeter(
          {"fcp", "envelope", "invalidsig"}, "envelope"))
    , mValueValid(app.getMetrics().NewMeter({"fcp", "value", "valid"}, "value"))
    , mValueInvalid(
          app.getMetrics().NewMeter({"fcp", "value", "invalid"}, "value"))
    , mValueExternalize(
          app.getMetrics().NewMeter({"fcp", "value", "externalize"}, "value"))
    , mQuorumHeard(
          app.getMetrics().NewMeter({"fcp", "quorum", "heard"}, "quorum"))
    , mNominatingValue(
          app.getMetrics().NewMeter({"fcp", "value", "nominating"}, "value"))
    , mUpdatedCandidate(
          app.getMetrics().NewMeter({"fcp", "value", "candidate"}, "value"))
    , mStartBallotProtocol(
          app.getMetrics().NewMeter({"fcp", "ballot", "started"}, "ballot"))
    , mAcceptedBallotPrepared(app.getMetrics().NewMeter(
          {"fcp", "ballot", "accepted-prepared"}, "ballot"))
    , mConfirmedBallotPrepared(app.getMetrics().NewMeter(
          {"fcp", "ballot", "confirmed-prepared"}, "ballot"))
    , mAcceptedCommit(app.getMetrics().NewMeter(
          {"fcp", "ballot", "accepted-commit"}, "ballot"))
    ...
    , mNominateToPrepare(
          app.getMetrics().NewTimer({"fcp", "timing", "nominated"}))
    , mPrepareToExternalize(
          app.getMetrics().NewTimer({"fcp", "timing", "externalized"}))
{
}
...
void
HerderSCPDriver::restoreSCPState(uint64_t index, fotonoValue const& value)
{
                                   ...
                                   std::set<Value> const& candidates)
{
    ...
    fotonoValue comp(h, 0, emptyUpgradeSteps, 0);
    ...
    std::vector<fotonoValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        fotonoValue& sv = candidateValues.back();
    ...
    if (removed.size() != 0)
    {
        ...
        // post to avoid triggering FCP handling code recursively
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid fotonoValue.
        CLOG(ERROR, "Herder") << "HerderSCPDriver::valueExternalized"
                              << " Externalized fotonoValue malformed";
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, fotonoValue const& value,
                          TxSetFramePtr proposedSet,
                          fotonoValue const& previousValue)
{
