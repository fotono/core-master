...
NominationProtocol::isSane(SCPStatement const& st)
{
    ...
    res = res && (std::adjacent_find(
                      ...
                      [](fotono::Value const& l, fotono::Value const& r) {
                          return !(l < r);
                      }) == nom.votes.end());
    res = res && (std::adjacent_find(
...
NominationProtocol::updateRoundLeaders()
{
    ...
    CLOG(DEBUG, "FCP") << "updateRoundLeaders: " << mRoundLeaders.size();
...
{
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "NominationProtocol::nominate (" << mRoundNumber
    ...
    {
        CLOG(DEBUG, "FCP") << "NominationProtocol::nominate (TIMED OUT)";
        return false;
    }
    ...
    {
        CLOG(DEBUG, "FCP") << "NominationProtocol::nominate (SKIPPED)";
    }
