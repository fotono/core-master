...
Peer::Peer(Application& app, PeerRole role)
    ...
    , mRecvGetSCPQuorumSetTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "get-fcp-qset"}))
    , mRecvSCPQuorumSetTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "fcp-qset"}))
    , mRecvSCPMessageTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "fcp-message"}))
    , mRecvGetSCPStateTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "get-fcp-state"}))

    , mRecvSCPPrepareTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "fcp-prepare"}))
    , mRecvSCPConfirmTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "fcp-confirm"}))
    , mRecvSCPNominateTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "fcp-nominate"}))
    , mRecvSCPExternalizeTimer(
          app.getMetrics().NewTimer({"overlay", "recv", "fcp-externalize"}))
    ...
    , mSendGetSCPQuorumSetMeter(app.getMetrics().NewMeter(
          {"overlay", "send", "get-fcp-qset"}, "message"))
    , mSendSCPQuorumSetMeter(
          app.getMetrics().NewMeter({"overlay", "send", "fcp-qset"}, "message"))
    , mSendSCPMessageSetMeter(app.getMetrics().NewMeter(
          {"overlay", "send", "fcp-message"}, "message"))
    , mSendGetSCPStateMeter(app.getMetrics().NewMeter(
          {"overlay", "send", "get-fcp-state"}, "message"))
...
{
    CLOG(TRACE, "Overlay") << "Get FCP State for " << ledgerSeq;
...
static std::string
msgSummary(fotonoMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(fotonoMessage const& msg)
{
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            << "("
...
void
Peer::recvMessage(fotonoMessage const& fotonoMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(fotonoMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (fotonoMsg.type() != HELLO) &&
        (fotonoMsg.type() != AUTH) && (fotonoMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << fotonoMsg.type() << " before completed handshake";
        mDropInRecvMessageUnauthMeter.Mark();
        drop();
        return;
    }

    assert(isAuthenticated() || fotonoMsg.type() == HELLO ||
           fotonoMsg.type() == AUTH || fotonoMsg.type() == ERROR_MSG);

    switch (fotonoMsg.type())
    {
    case ERROR_MSG:
    {
    ...
    case HELLO:
    {
        ...
        this->recvHello(fotonoMsg.hello());
    }
    break;
...
void
Peer::recvDontHave(fotonoMessage const& msg)
{
...
void
Peer::recvGetTxSet(fotonoMessage const& msg)
{
    auto self = shared_from_this();
...
void
Peer::recvTxSet(fotonoMessage const& msg)
{
...
void
Peer::recvTransaction(fotonoMessage const& msg)
{
...
void
Peer::recvGetSCPQuorumSet(fotonoMessage const& msg)
{
...
void
Peer::recvSCPQuorumSet(fotonoMessage const& msg)
{
...
void
Peer::recvSCPMessage(fotonoMessage const& msg)
{
...
void
Peer::recvGetSCPState(fotonoMessage const& msg)
{
    uint32 seq = msg.getSCPLedgerSeq();
    CLOG(TRACE, "Overlay") << "get FCP State " << seq;
    mApp.getHerder().sendSCPStateToPeer(seq, shared_from_this());
}

void
Peer::recvError(fotonoMessage const& msg)
{
...
void
Peer::recvAuth(fotonoMessage const& msg)
{
    if (mState != GOT_HELLO)
    {
    ...
    // ask for FCP state if not synced
    sendGetScpState(mApp.getLedgerManager().getLastClosedLedgerNum() + 1);
}

void
Peer::recvGetPeers(fotonoMessage const& msg)
{
    sendPeers();
}

void
Peer::recvPeers(fotonoMessage const& msg)
{
