# Transactions
See [Concept documentation](https://www.fotono.org/developers/guides/concepts/transactions.html).

Anything that changes the ledger is called a _Transaction_.
Transactions have an arbitrary list of operations inside them.

See the "struct Transaction" definition in src/xdr/fotono-transaction.x
...
When FCP externalizes the transaction set to apply to the last closed ledger:
...
## List of operations
See `src/xdr/fotono-transaction.x` for a detailed list of all operations and results.
