...
catching up to network:
    1) Wait for FCP to tell us what the network is on now
...
LedgerManagerImpl::valueExternalized(LedgerCloseData const& ledgerData)
{
    CLOG(INFO, "Ledger")
        ...
        << ", sv: " << fotonoValueToString(ledgerData.getValue()) << "]";
    ...
    else
    {
        ...
        for (auto const& lcd : mSyncingLedgers)
        {
            ...
            CLOG(INFO, "Ledger")
                ...
                << ", sv: " << fotonoValueToString(lcd.getValue()) << "]";
            closeLedger(lcd);
        }
...
the close context that was computed by FCP or by the historical module
during replays.
