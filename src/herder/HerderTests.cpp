...
TEST_CASE("FCP Driver", "[herder]")
{
    ...
    auto makeTxPair = [](TxSetFramePtr txSet, uint64_t closeTime) {
        txSet->sortForHash();
        auto sv = fotonoValue{txSet->getContentsHash(), closeTime,
                               emptyUpgradeSteps, 0};
...
TEST_CASE("FCP State", "[herder]")
{
    ...
    // but for "Force FCP" test there are 3 nodes and only 2 have previous
    ...
    auto doTest = [&](bool forceSCP) {
        ...
        // crank a bit (nothing should happen, node 2 is waiting for FCP
        // messages)
        ...
        // nodes 0 and 1 have lost their FCP state as they got restarted
...
TEST_CASE("In quorum filtering", "[herder]")
{
    ...
    // process fcp messages for each core node
    auto checkCoreNodes =
