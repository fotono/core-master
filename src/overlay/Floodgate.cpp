...
namespace fotono
{
Floodgate::FloodRecord::FloodRecord(fotonoMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(fotonoMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(fotonoMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
