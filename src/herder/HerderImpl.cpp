...
HerderImpl::SCPMetrics::SCPMetrics(Application& app)
    : mLostSync(app.getMetrics().NewMeter({"fcp", "sync", "lost"}, "sync"))
    , mBallotExpire(
          app.getMetrics().NewMeter({"fcp", "ballot", "expire"}, "ballot"))
    , mEnvelopeEmit(
          app.getMetrics().NewMeter({"fcp", "envelope", "emit"}, "envelope"))
    , mEnvelopeReceive(
          app.getMetrics().NewMeter({"fcp", "envelope", "receive"}, "envelope"))

    , mKnownSlotsSize(
          app.getMetrics().NewCounter({"fcp", "memory", "known-slots"}))
    , mCumulativeStatements(app.getMetrics().NewCounter(
          {"fcp", "memory", "cumulative-statements"}))
...
{
    CLOG(INFO, "Herder") << "Force joining FCP with local state";
...
void
HerderImpl::valueExternalized(uint64 slotIndex, fotonoValue const& value)
{
    // record metrics
    ...
    // the overhead of externalization in the way we track FCP)
    trackingHeartBeat();
}
...
HerderImpl::ledgerClosed()
{
    ...
    // If we are not a validating node and just watching FCP we don't call
...
HerderImpl::triggerNextLedger(uint32_t ledgerSeqToTrigger)
{
    ...
    // can be answered. Note this can trigger FCP callbacks, externalize, etc
    ...
    fotonoValue newProposedValue(txSetHash, nextCloseTime, emptyUpgradeSteps,
                                  0);
...
HerderImpl::resolveNodeID(std::string const& s, PublicKey& retKey)
{
    ...
    if (!r)
    {
        if (s.size() > 1 && s[0] == '@')
        {
            ...
            // go through FCP messages of the previous ledger
...
HerderImpl::getJsonInfo(size_t limit)
{
    ...
    ret["fcp"] = getSCP().getJsonInfo(limit);
...
HerderImpl::persistSCPState(uint64 slot)
{
    ...
    // saves FCP messages and related data (transaction sets, quorum sets)
...
HerderImpl::restoreSCPState()
{
    ...
    catch (std::exception& e)
    {
        ...
        CLOG(INFO, "Herder") << "Error while restoring old fcp messages, "
