...
class Floodgate
{
    class FloodRecord
    {
      public:
        ...
        uint32_t mLedgerSeq;
        fotonoMessage mMessage;
        std::set<Peer::pointer> mPeersTold;

        FloodRecord(fotonoMessage const& msg, uint32_t ledger,
                    Peer::pointer peer);
    };
  ...
  public:
    ...
    bool addRecord(fotonoMessage const& msg, Peer::pointer fromPeer);

    void broadcast(fotonoMessage const& msg, bool force);
