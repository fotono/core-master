# What is a ledger?
A ledger represents the state of the fotono universe at a given point in time.
The first ledger in history is called the genesis ledger.

Every FCP round, consensus decides on which transaction set to apply to the
...
[`src/xdr/fotono-ledger.x`](../xdr/fotono-ledger.x)
...
### Fields decided by consensus (FCP)
During consensus, nodes work together to decide on the value of fotonoValue.
fotonoValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'fotonoValue' should be able to transition to the same new ledger.
...
[`src/xdr/fotono-ledger-entries.x`](../xdr/fotono-ledger-entries.x)

### AccountEntry
This entry represents an account. In fotono, everything is centered around
...
Offers are a way to automate simple trading inside the fotono network.
...
* during normal operation, FCP calls the main "externalizeValue" method
