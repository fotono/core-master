...
class HerderSCPDriver : public SCPDriver
{
  public:
    struct ConsensusData
    {
        ...
        fotonoValue mConsensusValue;
        ConsensusData(uint64_t index, fotonoValue const& b)
    ...
    void restoreSCPState(uint64_t index, fotonoValue const& value);
    ...
    void nominate(uint64_t slotIndex, fotonoValue const& value,
                  TxSetFramePtr proposedSet, fotonoValue const& previousValue);
  ...
  private:
    ...
    // if the local instance is tracking the current state of FCP
    ...
    validateValueHelper(uint64_t slotIndex, fotonoValue const& sv) const;
