...
#define fotono_CORE_REAL_TIMER_FOR_CERTAIN_NOT_JUST_VIRTUAL_TIME
...
ApplicationImpl::getJsonInfo()
{
    ...
    info["build"] = fotono_CORE_VERSION;
    ...
    mLedgerManager->loadLastKnownLedger(
        [this, &done](asio::error_code const& ec) {
            ...
            if (mConfig.FORCE_SCP)
            {
                ...
                {
                    flagClearedMsg = " (`force fcp` flag cleared in the db)";
                    mPersistentState->setState(
                        PersistentState::kForceSCPOnNextLaunch, "false");
                }

                LOG(INFO) << "* ";
                LOG(INFO) << "* Force-starting fcp from the current db state."
                          << flagClearedMsg;
                LOG(INFO) << "* ";
