                                 ...
                                 SCPStatement const& st)
{
    ...
    // total ordering described in FCP paper.
...
{
    FCP::EnvelopeState res = FCP::EnvelopeState::INVALID;
    ...
    if (!isStatementSane(statement, self))
    {
        if (self)
        {
            CLOG(ERROR, "FCP") << "not sane statement from self, skipping "
    ...
    if (!isNewerStatement(nodeID, statement))
    {
        if (self)
        {
            CLOG(ERROR, "FCP") << "stale statement from self, skipping "
                               << "  e: " << mSlot.getSCP().envToStr(envelope);
        }
        else
        {
            CLOG(TRACE, "FCP") << "stale statement, skipping "
    ...
    else
    {
        ...
        if (self)
        {
            CLOG(ERROR, "FCP") << "invalid value from self, skipping "
                               << "  e: " << mSlot.getSCP().envToStr(envelope);
        }
        else
        {
            CLOG(TRACE, "FCP") << "invalid value "
                               << " i: " << mSlot.getSlotIndex();
        }
...
BallotProtocol::isStatementSane(SCPStatement const& st, bool self)
{
    ...
    if (!res)
    {
        CLOG(DEBUG, "FCP") << "Invalid quorum set received";
        return false;
    }
    ...
    case SCPStatementType::SCP_ST_PREPARE:
    {
        ...
        if (!isOK)
        {
            CLOG(DEBUG, "FCP") << "Malformed PREPARE message";
            res = false;
        }
    }
    break;
    case SCPStatementType::SCP_ST_CONFIRM:
    {
        ...
        if (!res)
        {
            CLOG(DEBUG, "FCP") << "Malformed CONFIRM message";
        }
    }
    break;
    case SCPStatementType::SCP_ST_EXTERNALIZE:
    {
        ...
        if (!res)
        {
            CLOG(DEBUG, "FCP") << "Malformed EXTERNALIZE message";
        }
    }
    break;
    default:
...
{
    CLOG(DEBUG, "FCP") << "BallotProtocol::abandonBallot";
    bool res = false;
...
BallotProtocol::bumpState(Value const& value, uint32 n)
{
    ...
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "BallotProtocol::bumpState"
                           << " i: " << mSlot.getSlotIndex()
...
BallotProtocol::updateCurrentValue(SCPBallot const& ballot)
{
    ...
    if (updated)
    {
        CLOG(TRACE, "FCP") << "BallotProtocol::updateCurrentValue updated";
    }
...
{
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "BallotProtocol::bumpToBallot"
                           << " i: " << mSlot.getSlotIndex()
...
BallotProtocol::emitCurrentStateStatement()
{
    ...
    {
        if (mSlot.processEnvelope(envelope, true) == FCP::EnvelopeState::VALID)
        {
            if (canEmit &&
...
{
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "BallotProtocol::setPreparedAccept"
                           << " i: " << mSlot.getSlotIndex()
                           << " b: " << mSlot.getSCP().ballotToStr(ballot);
...
{
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "BallotProtocol::setPreparedConfirmed"
                           << " i: " << mSlot.getSlotIndex()
...
{
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "BallotProtocol::setAcceptCommit"
                           << " i: " << mSlot.getSlotIndex()
...
{
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "BallotProtocol::setConfirmCommit"
                           << " i: " << mSlot.getSlotIndex()
                           << " new c: " << mSlot.getSCP().ballotToStr(c)
                           << " new h: " << mSlot.getSCP().ballotToStr(h);
...
BallotProtocol::advanceSlot(SCPStatement const& hint)
{
    ...
    if (Logging::logDebug("FCP"))
        CLOG(DEBUG, "FCP") << "BallotProtocol::advanceSlot "
