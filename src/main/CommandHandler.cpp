...
CommandHandler::CommandHandler(Application& app) : mApp(app)
{
    ...
    addRoute("fcp", &CommandHandler::scpInfo);
...
{
    retStr = "<b>Welcome to fotono-core!</b><p>";
    ...
    retStr +=
        ...
        "</p><p><h1> /fcp?[limit=n]</h1>"
        "returns a JSON object with the internal state of the FCP engine for "
        ...
        "Cursors are used by dependent services to tell fotono - core which "
...
CommandHandler::manualClose(std::string const& params, std::string& retStr)
{
    ...
    {
        retStr =
            "Set MANUAL_CLOSE=true in the fotono-core.cfg if you want this "
            "behavior";
    }
}
...
CommandHandler::generateLoad(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        retStr = "Set ARTIFICIALLY_GENERATE_LOAD_FOR_TESTING=true in "
                 "the fotono-core.cfg if you want this behavior";
    }
}
...
CommandHandler::ll(std::string const& params, std::string& retStr)
{
    ...
    if (!levelStr.size())
    {
        ...
        root["FCP"] = Logging::getStringFromLL(Logging::getLogLevel("FCP"));
        root["Bucket"] =
