...
PendingEnvelopes::PendingEnvelopes(Application& app, HerderImpl& herder)
    ...
    , mReadyEnvelopesSize(
          app.getMetrics().NewCounter({"fcp", "memory", "pending-envelopes"}))
{
}
...
{
    CLOG(TRACE, "Herder") << "Discarding FCP Envelopes with SCPQSet "
                          << hexAbbrev(hash);
...
PendingEnvelopes::isNodeInQuorum(NodeID const& node)
{
    ...
    else
    {
        ...
        FCP::TriBool r = mHerder.getSCP().isNodeInQuorum(node);
