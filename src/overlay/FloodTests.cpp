...
TEST_CASE("Flooding", "[flood][overlay]")
{
    ...
    SECTION("fcp messages flooding")
    {
        // FCP messages depend on
        // a quorum set
        ...
        auto injectSCP = [&](int i) {
            ...
            // build an FCP nomination message for the next ledger

            fotonoValue sv(txSet.getContentsHash(),
        ...
        auto ackedSCP = [&](std::shared_ptr<Application> app) {
            // checks if an app received and processed all FCP messages
            size_t okCount = 0;
            auto const& lcl =
