...
bool force_sqlite = (std::getenv("fotono_FORCE_SQLITE") != nullptr);
...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("fotono-core-test");
        ...
        sstream << "fotono" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    // not mixed with fotono-core logging.
    Logging::setFmt("<test>");
    ...
    LOG(INFO) << "Testing fotono-core " << fotono_CORE_VERSION;
    ...
    cli |= clara::Opt(gBaseInstance, "offset")["--base-instance"](
        ...
        "fotono-core can run tests concurrently");
    session.cli(cli);
